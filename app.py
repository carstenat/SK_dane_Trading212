...
if uploaded_files:
    ...
    if not zoznam_tickerov_all:
        ...
        vybrany_text = st.selectbox("Vyberte akciu zo svojho portfólia:", ponuka_pre_menu, key="widget_vyber_final_v51")
        vybrany_ticker_pure = mapovanie_tickerov[vybrany_text]
        
        col_input1, col_input2 = st.columns(2)
        with col_input1:
            skutocny_stav = st.number_input("Počet kusov vlastnených na platforme Trading 212:", min_value=0.0, value=0.0, step=0.00001, format="%.5f", key="vstup_pro_final_v51")
        with col_input2:
            aktualna_cena = st.number_input("Aktuálna trhová cena akcie (EUR):", min_value=0.0, value=0.0, step=0.01, format="%.2f", key="vstup_cena_final_v51")
        ...
        for _, riadok in df_ticker.iterrows():
            ...
            if 'buy' in typ or 'investment' in typ or 'deposit' in typ:
                # 🛡️ ABSOLÚTNA KOREKCIA: Ak je shares nula (split), riadok bezpečne ignorujeme a kód nepadne!
                if shares > 0.00001:
